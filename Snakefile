...
rule filtlong:
	...
	conda:
		"envs/conda.yaml"
	shell:
		"filtlong --min_length 2000 --max_length 6000  --min_mean_q 90 {input} > {output}"
...
rule racon:
	...
	threads: config["threads"]
	shell:
		"racon {input.fai} -q 20 -w 500 -t {threads} {input.paf} {input.con} > {output}"
